...
// Derived from FPSWalkerEnhanced

[RequireComponent(typeof(CharacterController))]
public class DogKnightController : MonoBehaviour
{
    [Tooltip("If true, auto run typing the W key.")]
    [SerializeField]
    ...
    void AnimUpdate(float xx, float yy)
    {
    ...
    private void Update()
    {
        ...
        if (m_AutoRun && inputY > 0f)
            clampY = true;
        if (clampY)
            inputY = 1f;


        AnimUpdate(inputX, inputY);
    }
