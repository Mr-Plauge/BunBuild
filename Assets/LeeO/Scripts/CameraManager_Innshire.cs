...
public class CameraManager_Innshire : MonoBehaviour {
	...
	{ //Function that makes the camera follow the player
		...
		Vector3 targetPosition = Vector3.Lerp(transform.position, new Vector3(target.position.x + offset.x, target.position.y + offset.y, target.position.z + offset.z), speed); //Bring the camera closer to the player interpolating with the velocity(0.5 half, 1 everything)
	...
	{ //Function that rotates the camera correctly
		...
		camTrans.transform.rotation = targetRotation;
	...
	public static CameraManager_Innshire singleton; //You can call CameraManager.singleton from other script (There can be only one)
	void Awake()
	{
